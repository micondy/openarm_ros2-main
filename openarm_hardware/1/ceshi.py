...
# 零位关节
q = zero(model.nq)
jid = model.getJointId("openarm_right_joint1")
q[model.joints[jid].idx_q] = 0.5

jid = model.getJointId("openarm_left_joint1")
q[model.joints[jid].idx_q] = 0.5
...
model = pin.buildModelFromUrdf("openarm_bimanual.urdf")

print("=== Pinocchio Joint Axis ===")

for i in range(1, model.njoints):
    joint = model.joints[i]
    name = model.names[i]

    print(i, name, joint.shortname())
